...
func TestActivityDumps(t *testing.T) {
	...
	t.Run("activity-dump-timeout", func(t *testing.T) {
		dockerInstance, dump, err := test.StartADockerGetDump()
		if err != nil {
			t.Fatal(err)
		}
		...
		time.Sleep((testActivityDumpCgroupDumpTimeout*60 - 1) * time.Second)
	...
	t.Run("activity-dump-cgroups-counts", func(t *testing.T) {
