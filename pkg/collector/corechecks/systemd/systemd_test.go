...
unit_names:
 ...
 - syslog.socket
`)
	err := check.Configure(1, rawInstanceConfig, []byte(``), "test")
...
func TestMissingUnitNamesShouldRaiseError(t *testing.T) {
	check := SystemdCheck{}
	err := check.Configure(1, []byte(``), []byte(``), "test")
...
`)
	...
	check.Configure(1, rawInstanceConfig, []byte(``), "test")
...
func TestDbusConnectionErr(t *testing.T) {
	...
	check := SystemdCheck{stats: stats}
	check.Configure(1, []byte(``), []byte(``), "test")
...
`)
	check := SystemdCheck{stats: stats}
	check.Configure(1, rawInstanceConfig, nil, "test")
...
`)
	...
	systemdCheck.Configure(1, rawInstanceConfig, nil, "test")
	// run
	systemdCheck.Run()
